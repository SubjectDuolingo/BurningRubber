...
public class CarCam : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        rootNode.position = Vector3.Lerp(rootNode.position, car.transform.position, cameraStickiness * Time.fixedDeltaTime);
        ...
        if (carPhysics.velocity.magnitude < rotationThreshold)
            look = Quaternion.LookRotation(car.transform.forward);
        else
